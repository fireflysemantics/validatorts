...
function currencyRegex(options:any) {
  ...
  options.digits_after_decimal.forEach((digit:any, index:any) => { if (index !== 0) decimal_digits = `${decimal_digits}|\\d{${digit}}`; });
  const symbol =
 ...
 */
export function isCurrency(str: string, options:any) {
  assertString(str);
